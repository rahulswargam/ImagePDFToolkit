...
from tools.excel_to_pdf import convert_excel_to_pdf
...
_FALLBACK_WARNING = (
    "Microsoft Excel isn't detected on this machine — using a simplified fallback "
    "converter. Print areas, page breaks, charts, and complex formatting may not be "
    "preserved exactly."
)


class ExcelToPdfPage(QWidget):
    ...
    def setup_ui(self):
        ...
        self.drop_workspace = DropWorkspace(
            ...
            _ACCENT,
            multiple=True,
            title="Drop Excel workbooks here",
            subtitle=f"Drag & drop up to {MAX_BATCH_FILES} .xlsx/.xls files, or click to browse",
            hint="XLSX · XLS",
        )
        ...
        self.fallback_note.setVisible(not is_office_app_installed("excel"))
    ...
    def select_files(self):
        ...
        file_paths, _ = QFileDialog.getOpenFileNames(
            self, "Select Excel Workbooks", "", "Excel Workbooks (*.xlsx *.xls)"
        )
    ...
    def load_files(self, file_paths):
        ...
        if truncated:
            CompletionDialog.warn(
                self,
                "Too Many Files",
                f"Only the first {MAX_BATCH_FILES} workbooks were kept "
    ...
    def refresh_summary(self):
        ...
        if not file_paths:
            ...
            self.drop_workspace.set_text(
                ...
                f"Drag & drop up to {MAX_BATCH_FILES} .xlsx/.xls files, or click to browse",
            )
            return

        plural = "workbook" if len(file_paths) == 1 else "workbooks"
        self.summary_label.setText(f"{len(file_paths)} {plural} selected")
        self.summary_label.show()

        self.convert_button.setEnabled(True)
        self.drop_workspace.set_text(f"{len(file_paths)} {plural} ready", "Drop more workbooks or click to add")
    ...
    def convert(self):
        ...
        for index, input_path in enumerate(self.input_paths, start=1):
            ...
            try:
                self._last_output_path, method = convert_excel_to_pdf(input_path)
                saved += 1
                if method == "fallback":
                    fallback_count += 1
        ...
        if saved:
            plural = "workbook" if saved == 1 else "workbooks"
            if fallback_count:
                dialog = CompletionDialog(
                    "Processing complete",
                    f"{saved} {plural} converted, but {fallback_count} of them used the "
                    "simplified fallback renderer (reduced fidelity) because Microsoft "
