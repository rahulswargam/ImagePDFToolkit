...
_FALLBACK_WARNING = (
    "Microsoft Word isn't detected on this machine — using a simplified fallback "
    "converter. Fonts, headers/footers, images, and complex formatting may not be "
    "preserved exactly."
)
...
class WordToPdfPage(QWidget):
    ...
    def setup_ui(self):
        ...
        self.drop_workspace = DropWorkspace(
            ...
            subtitle=f"Drag & drop up to {MAX_BATCH_FILES} .docx/.doc files, or click to browse",
            hint="DOCX · DOC",
        )
        ...
        self.fallback_note.setVisible(not is_office_app_installed("word"))
    ...
    def select_files(self):
        ...
        file_paths, _ = QFileDialog.getOpenFileNames(
            self, "Select Word Documents", "", "Word Documents (*.docx *.doc)"
        )
    ...
    def load_files(self, file_paths):
        ...
        if truncated:
            CompletionDialog.warn(
                self,
                "Too Many Files",
                f"Only the first {MAX_BATCH_FILES} documents were kept "
    ...
    def refresh_summary(self):
        ...
        if not file_paths:
            ...
            self.drop_workspace.set_text(
                ...
                f"Drag & drop up to {MAX_BATCH_FILES} .docx/.doc files, or click to browse",
            )
            return

        plural = "document" if len(file_paths) == 1 else "documents"
        self.summary_label.setText(f"{len(file_paths)} {plural} selected")
        self.summary_label.show()

        self.convert_button.setEnabled(True)
        self.drop_workspace.set_text(f"{len(file_paths)} {plural} ready", "Drop more documents or click to add")
    ...
    def convert(self):
        ...
        for index, input_path in enumerate(self.input_paths, start=1):
            ...
            try:
                self._last_output_path, method = convert_word_to_pdf(input_path)
                saved += 1
                if method == "fallback":
                    fallback_count += 1
        ...
        if saved:
            plural = "document" if saved == 1 else "documents"
            if fallback_count:
