...
class PdfToWordPage(QWidget):
    ...
    def setup_ui(self):
        ...
        self.file_grid = FileGrid(lambda path: "pdf", self._file_meta)
        self.file_grid.filesChanged.connect(self.on_files_changed)
        main_layout.addWidget(self.file_grid)

        note = QLabel(
            "Reconstructs text, tables, and images page by page. Complex layouts "
        ...
        self.convert_button = AnimatedButton("Convert to Word")
    ...
    def convert(self):
        ...
        for index, input_path in enumerate(self.input_paths, start=1):
            ...
            try:
                self._last_output_path = convert_pdf_to_word(input_path)
                saved += 1
        ...
        if saved:
            ...
            CompletionDialog.success(
                self,
                "Processing complete",
                f"{saved} {plural} converted into Word documents.",
